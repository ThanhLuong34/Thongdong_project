<?php $pageTitle = "Giới thiệu - Thong Dong"; ?>
<?php include '../includes/customer-layout-top.php'; ?>

<main class="container" style="padding:34px 0 70px;">

  <!-- HERO -->
  <section class="card about-hero">
    <div class="about-hero-left">
      <h1 style="margin:0 0 10px;">Giới thiệu</h1>
      <p class="muted" style="margin:0 0 14px;">
        Thong Dong là một góc nhỏ dành cho những điều an yên — mùi hương thuần Việt cho người Việt.
      </p>
      <div class="about-cta">
        <a class="btn" href="/thongdong/customer/shop.php">Xem cửa hàng</a>
        <a class="btn outline" href="/thongdong/customer/blog.php">Đọc Nhật ký</a>
      </div>
    </div>

    <div class="about-hero-right">
      <div class="about-photo">
        <span>Ảnh/Poster thương hiệu</span>
      </div>
    </div>
  </section>

  <!-- STORY -->
  <section class="card about-section">
    <h2 class="about-title">Câu chuyện Thong Dong</h2>
    <p>
      Có những ngày mình chỉ muốn chậm lại một chút. Thong Dong ra đời từ những khoảnh khắc như vậy —
      khi căn phòng nhỏ cần một mùi hương ấm, một ngọn lửa dịu, để lòng mình “thở” nhẹ hơn.
    </p>
    <p>
      Tụi mình chọn những mùi hương gần gũi: <b>sen, bưởi, trà, quế, gừng, gỗ mộc</b>…
      để mỗi lần thắp nến là một lần nhắc mình: “Bình yên cũng là một lựa chọn.”
    </p>
  </section>

  <!-- VALUES -->
  <section class="about-grid">
    <article class="card about-card">
      <h3>Thuần Việt</h3>
      <p class="muted">Mùi hương gợi ký ức Việt: mộc mạc, ấm áp, dễ chịu.</p>
    </article>

    <article class="card about-card">
      <h3>Đủ tinh tế</h3>
      <p class="muted">Thiết kế tối giản, sang và ấm — đúng vibe Tết, đúng vibe nhà.</p>
    </article>

    <article class="card about-card">
      <h3>Gói quà đẹp</h3>
      <p class="muted">Set quà nhỏ xinh, phù hợp tặng bạn bè, gia đình, đồng nghiệp.</p>
    </article>
  </section>

  <!-- COMMIT -->
  <section class="card about-section">
    <h2 class="about-title">Cam kết nhỏ</h2>
    <ul class="about-list">
      <li><b>Thơm dễ chịu</b> — không gắt, hợp không gian sinh hoạt.</li>
      <li><b>Cháy đều</b> — gợi ý cách thắp đúng để nến bền hơn.</li>
      <li><b>Hỗ trợ nhanh</b> — cần tư vấn mùi hương, tụi mình trả lời liền.</li>
    </ul>
  </section>

  <!-- (NEW) ĐẾN TIỆM -->
  <section class="card about-section visit">
    <div class="visit-left">
      <h2 class="about-title">Đến tiệm</h2>
      <p class="muted" style="margin-top:0;">
        Nếu bà muốn thử mùi trực tiếp, ghé Thong Dong nha.
      </p>

      <div class="visit-item">
        <div class="muted">Địa chỉ</div>
        <div><b>123 Đường Hoa Mai, Quận Hải Châu, Đà Nẵng</b></div>
      </div>

      <div class="visit-item">
        <div class="muted">Giờ mở cửa</div>
        <div><b>09:00 – 21:00</b> (Thứ 2 – Chủ nhật)</div>
      </div>

      <div class="visit-item">
        <div class="muted">Hotline</div>
        <div><b>0900 000 000</b></div>
      </div>

      <div class="visit-item">
        <div class="muted">Mạng xã hội</div>
        <div>
          <a href="#" class="link">Facebook</a>
          <span class="muted"> • </span>
          <a href="#" class="link">Instagram</a>
          <span class="muted"> • </span>
          <a href="#" class="link">TikTok</a>
        </div>
      </div>

      <div class="visit-actions">
        <a class="btn" href="/thongdong/customer/shop.php">Mua online</a>
        <a class="btn outline" href="https://maps.google.com" target="_blank">Mở Google Maps</a>
      </div>
    </div>

    <div class="visit-right">
      <div class="map-box">
        <span>Bản đồ/Ảnh tiệm</span>
      </div>
    </div>
  </section>

  <!-- (NEW) MINI GALLERY -->
  <section class="card about-section">
    <h2 class="about-title">Một vài khoảnh khắc</h2>
    <p class="muted" style="margin-top:0;">Vibe vàng – đỏ – gạch bông, rất “Tết” và rất Việt.</p>

    <div class="gallery-grid">
      <div class="gallery-item">
        <img src="/thongdong/assets/img/about/gallery-1.jpg" alt="Thong Dong gallery 1">
      </div>
      <div class="gallery-item">
        <img src="/thongdong/assets/img/about/gallery-2.jpg" alt="Thong Dong gallery 2">
      </div>
      <div class="gallery-item">
        <img src="/thongdong/assets/img/about/gallery-3.jpg" alt="Thong Dong gallery 3">
      </div>
    </div>

    <div class="gallery-note muted">
      * Nếu chưa có ảnh, bạn có thể copy tạm 1 ảnh bất kỳ đặt đúng tên file để khỏi bị lỗi.
    </div>
  </section>

  <!-- (NEW) FORM LIÊN HỆ NHANH -->
  <section class="card about-section contact-quick">
    <h2 class="about-title">Liên hệ nhanh</h2>
    <p class="muted" style="margin-top:0;">
      Bạn để lại lời nhắn, tụi mình sẽ phản hồi sớm nha.
    </p>

    <form class="contact-form" method="post" action="#">
      <div class="form-row">
        <div class="form-group">
          <label>Họ và tên</label>
          <input class="input" type="text" name="full_name" placeholder="Ví dụ: Tiên">
        </div>
        <div class="form-group">
          <label>Số điện thoại</label>
          <input class="input" type="text" name="phone" placeholder="Ví dụ: 09xx xxx xxx">
        </div>
      </div>

      <div class="form-row">
        <div class="form-group">
          <label>Email (tuỳ chọn)</label>
          <input class="input" type="email" name="email" placeholder="vidu@gmail.com">
        </div>
        <div class="form-group">
          <label>Chủ đề</label>
          <select class="input" name="topic">
            <option value="tu-van">Tư vấn mùi hương</option>
            <option value="don-hang">Hỏi về đơn hàng</option>
            <option value="qua-tang">Gói quà / set quà</option>
            <option value="khac">Khác</option>
          </select>
        </div>
      </div>

      <div class="form-group">
        <label>Lời nhắn</label>
        <textarea class="input" name="message" rows="4" placeholder="Bạn muốn tư vấn mùi nào, dùng phòng nào, mood ra sao..."></textarea>
      </div>

      <div class="contact-actions">
        <button class="btn" type="submit">Gửi liên hệ</button>
        <a class="btn outline" href="/thongdong/customer/shop.php">Xem cửa hàng</a>
      </div>

      <div class="muted" style="margin-top:10px;">
      </div>
    </form>
  </section>

  <!-- CTA -->
  <section class="card about-banner">
    <div>
      <h2 style="margin:0 0 8px;">Bạn muốn chọn mùi hương hợp mood?</h2>
      <p class="muted" style="margin:0;">Vào cửa hàng và thử lọc theo bộ sưu tập nha.</p>
    </div>
    <a class="btn" href="/thongdong/customer/shop.php">Mua ngay</a>
  </section>

</main>

<?php include '../includes/customer-layout-bottom.php'; ?>
